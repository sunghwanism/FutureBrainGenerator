
import argparse


################### LDMCf Configuration ###################

def LDMCf_get_run_parser():
    parser = argparse.ArgumentParser()
    model = 'LDMcf'

    ####################### BASE Configuration #######################
    # Seed
    parser.add_argument('--seed', type=int, default=42,
                        help='Random seed')
    
    # Model
    parser.add_argument('--train_model', type=str, default='LDMcf',
                        help='Which model to run')
    
    # PATH
    parser.add_argument('--base_path', type=str, default='/NFS/ADDiff',)
    
    # Data
    parser.add_argument('--data_path', type=str, default=f'/data/moonsh/ADD_diff/LDM/', # same with LDM
                        help='Path to data')
    parser.add_argument('--crop_size',type=int, nargs='+', default=(96, 128, 96),)
    
    # Device Arguments
    parser.add_argument('--device_id', type=str,
                        help='Which GPU to use')
    parser.add_argument('--num_workers', type=int, default=5,
                        help='Number of workers for dataloader')

    ####################### VQVAE Configuration #######################

    # Hyperparameters
    parser.add_argument('--num_embeddings', type=int, default=1024) # code book size
    parser.add_argument('--latent_channels', type=int, default=8)
    parser.add_argument('--num_channels', type=int, nargs='+', default=(64, 128, 256), help="List of channel sizes")
    # parser.add_argument('--num_channels', type=int, nargs='+', default=(256,512), help="List of channel sizes")
    parser.add_argument('--num_res_blocks', type=int, default=2)
    parser.add_argument('--commitment_cost', type=float, default=0.5) # base = 0.25
    parser.add_argument('--downsample_param', type=int, nargs='+', 
                        default=((1, 3, 1, 1), (2, 4, 1, 1), (2, 4, 1, 1)),
                        # default=((2, 4, 1, 1), (2, 4, 1, 1)),
                        help='list of (stride, kernel, dilation, padding)')
    parser.add_argument('--upsample_param', type=int, nargs='+', 
                        default=((1, 3, 1, 1, 0), (2, 4, 1, 1, 0), (2, 4, 1, 1, 0),))
                        # default = ((2, 4, 1, 1, 0), (2, 4, 1, 1, 0),))


    # downsample_parameters=((2, 4, 1, 1), (2, 4, 1, 1)),
    # upsample_parameters=((2, 4, 1, 1, 0), (2, 4, 1, 1, 0)),
    ###################### Classifier Configuration #######################

    # Hyperparameters
    parser.add_argument('--num_classes', type=int, default=3) # HC, MCI, AD    
    parser.add_argument('--clf_num_channels', type=int, nargs='+', default=(128, 256), help="List of channel sizes")
    parser.add_argument('--clf_num_res_blocks', type=int, nargs='+', default=(2, 2))
    parser.add_argument('--clf_num_head_channels', type=int, nargs='+', default=(0, 64),)
    parser.add_argument('--clf_attention_levels', type=int, nargs='+', default=(0, 1),)

    ####################### LDMCf Configuration #######################

    # Finetuning Arguments
    parser.add_argument('--encoder_all_freeze', action='store_true',
                        help='Freeze all encoder layers')
    
    # Train Arguments
    parser.add_argument('--batch_size', type=int, default=12,
                        help='Batch size')
    parser.add_argument('--epochs', type=int, default=1000,
                        help='Number of epochs')    
    parser.add_argument('--unet_lr', type=float, default=1e-4,
                        help='Generator Learning rate')
    parser.add_argument('--diff_weight', type=float, default=0.99,)
    parser.add_argument('--clf_weight', type=float, default=0.01,)
    parser.add_argument('--condition', nargs='+', default=['Age', 'Sex', 'MMSE', 'Education', 'interval'],
                        help='Condition for classifier')
    parser.add_argument('--use_clf', action='store_true',
                        help='Use classifier')
    parser.add_argument('--use_baseimg', action='store_true',
                        help='Use base image as condition')
    
    # Diffusion Scheduler Arguments
    parser.add_argument('--scheduler', default='ddpm', # or ddim
                        help='Scheduler')
    parser.add_argument('--schedule_type', type=str, default='scaled_linear_beta',)

    parser.add_argument('--timestep', type=int, default=1000,)

    parser.add_argument('--beta_start', type=float, default=0.0015,) # 0.0015
    parser.add_argument('--beta_end', type=float, default=0.0195,) # 0.0195 

    # Hyperparameters
    parser.add_argument('--diff_num_channels', type=int, nargs='+', default=(128, 256, 512), help="List of channel sizes")
    parser.add_argument('--diff_num_res_blocks', type=int, default=1)
    parser.add_argument('--diff_num_head_channels', type=int, nargs='+', default=(0, 128, 256),)
    parser.add_argument('--diff_attention_levels', type=int, nargs='+', default=(0, 1, 1),)

    parser.add_argument('--use_pretrain_clf', action='store_true',
                        help='Use pretrain classifier')

    ####################### SAVE Configuration #######################

    # Save and Log Arguments
    parser.add_argument('--save_path', type=str, default=f'/NFS/ADDiff/ckpt/{model}',
                        help='Where to save the model')
    parser.add_argument('--save_img_path', type=str, default=f'/NFS/ADDiff/img/{model}',
                        help='Where to save the images')
    parser.add_argument('--save_interval', type=int, default=10,
                        help='How often to save')
    
    parser.add_argument('--save_img_interval', type=int, default=100,
                        help='How often to save images')
    parser.add_argument('--n_example_images', type=float, default=2,
                        help='Validation images')
    
    # Wandb Arguments
    parser.add_argument('--wandb_project', type=str, default='AD_diff_LDMClf_QC',
                        help='Wandb project')
    parser.add_argument('--wandb_entity', type=str, default='msh2044',
                        help='Wandb entity')
    parser.add_argument('--nowandb', action='store_true',
                        help='Don\'t use wandb')
    
    return parser

########################################################################################################

