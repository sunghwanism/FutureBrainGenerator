...
class DiffusionModelClfUNet(nn.Module):
    """
    Unet network with timestep embedding and attention mechanisms for conditioning based on
    Rombach et al. "High-Resolution Image Synthesis with Latent Diffusion Models" https://arxiv.org/abs/2112.10752
    and Pinaya et al. "Brain Imaging Generation with Latent Diffusion Models" https://arxiv.org/abs/2209.07162
    ...
    # New explanation
    We added classifier guidance to the model, so that the model can be used for classification tasks.
    ...
    def __init__(
        ...
        if classifier is not None:
            self.img_condition_mixer = nn.Sequential(nn.Linear(cross_attention_dim-3, 64),
                                                    nn.ReLU(),
                                                    nn.Linear(64, cross_attention_dim-3),)
        else:
            self.img_condition_mixer = nn.Sequential(nn.Linear(cross_attention_dim, 64),
                                                    nn.ReLU(),
                                                    nn.Linear(64, cross_attention_dim),)

    def process_base_img(self, condition, x_shape):
        
        base_img = condition[:, :, :-5]
        base_img = base_img.view(x_shape[0], -1, *x_shape[2:])
        condition = condition[:,:, -5:]
        base_img = F.adaptive_avg_pool3d(base_img, (1, 1, 1))
        base_img = base_img.view(base_img.size(0), 1, -1)

        condition = torch.cat((base_img, condition), dim=2)
    ...
    def forward(
        ...
        if use_baseimg:
            context = self.process_base_img(context, x.shape)
            context_witout_interval = context[:, :, :-1] # remove interval for classifier

            context = self.img_condition_mixer(context)
        
        # 3. initial convolution
        h = self.conv_in(x)
        
        if self.classifier is not None: # To-do: Do not change emb
            logits = self.classifier(x, emb, context_witout_interval)
            with torch.no_grad():
                prob = torch.nn.functional.softmax(logits, dim=1).unsqueeze(1)
                context = torch.cat((context, prob), dim=2)
        else:
            logits = None
        ...
        if context is not None and self.with_conditioning is False:
            raise ValueError("model should have with_conditioning = True if context is provided")
        down_block_res_samples: list[torch.Tensor] = [h]
        for h_i, downsample_block in enumerate(self.down_blocks):
            h, res_samples = downsample_block(hidden_states=h, temb=emb, context=context)
            for residual in res_samples:
                down_block_res_samples.append(residual)
        ...
        # 6. up
        for h_i, upsample_block in enumerate(self.up_blocks):
            res_samples = down_block_res_samples[-len(upsample_block.resnets) :]
            down_block_res_samples = down_block_res_samples[: -len(upsample_block.resnets)]
            h = upsample_block(hidden_states=h, res_hidden_states_list=res_samples, temb=emb, context=context)
